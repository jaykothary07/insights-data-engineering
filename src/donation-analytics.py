...
def find_political_donors(inputf,outputz,outputd):
	...
	inheaders2 = ["percentile"]
	per = pd.read_csv(outzipfile, names=inheaders2,converters={'percentile': lambda x: int(x)})
	#print(per)
	#Remove bad values
	...
	if (any(pd.isnull(indata.NAME))):
		indata = indata[pd.isnull(indata.NAME) == False].reset_index(drop=True)  # Check for missing Namee
	...
	for x in range(len(indata)):
		...
		in_name = indata.NAME[x]
		in_CMTE_ID = indata.CMTE_ID[x]
		in_DT = validate_date(indata.TRANSACTION_DT[x])
		in_per =per.iloc[0]['percentile']
		#print(in_per)
		#Update databank
		DataBank=DataBank.append(pd.DataFrame([[in_CMTE_ID,in_name,in_zip,in_DT,indata.TRANSACTION_AMT[x]]],columns=DB_header),ignore_index=True)
		print(DataBank)
		if int(in_zip)>0: #If ZIPCODE is invalid ignore for zip file
			#Get values for zipcode file
			ziptrans=DataBank.TRANSACTION_AMT[(DataBank.CMTE_ID==in_CMTE_ID) & (DataBank.ZIP_CODE==in_zip) & (DataBank.TRANSACTION_DT==2018)] # get transactions for zip and CMTE
			#print(ziptrans)
			#calculate vals
			if in_DT==2018:
			...
			if in_DT==2018:
				...
				file.write(in_CMTE_ID + '|' + str(in_zip) + '|' + str(in_DT) + '|' + str(int(percent) ) + '|' + str(int(tot_fromzip)) + '|' + str(num_fromzip) + '\n')
				file.close()
		...
		#date_df=DataBank[pd.to_numeric(DataBank.TRANSACTION_DT)>0].groupby(['NAME', 'CMTE_ID', 'ZIP_CODE', 'TRANSACTION_DT']).agg({'TRANSACTION_AMT' :['median','count','sum']})
		...
		#date_df.to_csv(outdatefile, sep='|', columns = ['CMTE_ID'], header=False, index=True, index_label=None, mode='w', encoding=None, compression=None, quoting=None, quotechar='"', line_terminator='\n', chunksize=None, tupleize_cols=False, date_format=None, doublequote=True, escapechar=None, decimal='.')
